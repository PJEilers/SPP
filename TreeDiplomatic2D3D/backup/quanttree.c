...
int LocalTreeFlood(int self,  Queue *set, pixel_t *lero, int lev, long *thisarea, void **thisattr, MaxNode *nodes, int *gvalues, GeneralThreadData *threadData)
{
                ...
                else 
		  {
		    ...
		    nodes[q].parent = lero[fq];
                ...
                if (fq > lev)
                {
                    ...
                    do
                    {
                        fq = LocalTreeFlood(self, set, lero, fq, &childarea, &childattr, nodes, gvalues, threadData);
                        if(fq>=numQTZLEVELS)
                        {
                            DeleteAuxData(attr);
                            return(fq);
...
void *ccaf(void *arg)
{
    ...
    LocalTreeFlood(self, set, lero, gval_qu[xm], &area, &attr, node_qu, gval_qu, thdata);
    /*for (i=0; i<size; i++)
