...
int ImagePGMBinWrite(char *fname)
{
   ...
   /*   width = width/4; height = height/4; size = width * height;
        fprintf(outfile, "P5\n%ld %ld\n255\n", width, height);
        fwrite(gval, 1, (size_t)(size), outfile); */
...
int LocalTreeFlood(int self,  Queue *set, ulong *lero, int lev, ulong *thisarea, void **thisattr)
{
   ulong neighbors[CONNECTIVITY];
   ulong area = *thisarea, childarea;
   void *attr = NULL, *childattr;
   ulong p, q, x, y, lwb, upb;
   int fq;
      ...
      for (i=0; i<numneighbors; i++) {
         ...
         if (!reached[q]) {
            ...
            if (fq > lev) {
               ...
               childattr = NULL;
               do {
                  fq = LocalTreeFlood(self, set, lero, fq, &childarea, &childattr);
                  if (fq>=MAXLEVELS) {
                     DeleteAuxData(attr);
                     return(fq);
                  }
...
ulong levroot(ulong x) { /* pre: x!=bottom */
  if (x==bottom)
    return bottom;
...
void Connect (ulong x, ulong y, ulong lwb, ulong upb) {
...
void *ccaf(void *arg) {
  ...
  set = QueueCreate(imgsize, numpixelsperlevel);
  if (set==NULL) return(NULL);
  QueueAdd(set, gval[xm], xm);
  reached[xm] = true;
  lero[gval[xm]] = xm;             

  LocalTreeFlood(self, set, lero, gval[xm], &area, &attr);
  QueueDelete(set);
